...
toc_footers:
  - <a href='https://docs.google.com/document/d/1t7DazjpM2EF6Og8NDLoHKPGEuKWTvWdx9HgaJm-8WNw' target='_blank'>Первая версия документации API</a>
  - <a href='https://joinposter.com' target='_blank'>Перейти на сайт joinposter.com</a>
...
```php
$url = 'https://demo.joinposter.com/api/clients.changeClientBonus?' .
  'format=json&token=745d516e1b9320ed85b84d5bfda14148';
...
$bonus = sendRequest($url, 'post', $bonus);
```
...
Запрос позволяет изменить количество бонусов на счету клиента. В качестве количества необходимо прислать дельту, а не окончательное значение.
...
Параметр | Описание
--------- | -----------
client_id | id клиента, для которого меняем количество бонусов
count |  Количество бонусов (в гривнах/рублях), которые надо начилить клиенту, если count положительный, или списать, если count отрицательный
block_webhook | Если вы слушаете вебхуки по клиентам, то будет полезно прислать этот параметр в виде true, чтобы не получить в ответ вебхук об измении данных этого клиента

### Параметры ответа clients.changeClientBonus

Параметр | Описание
--------- | -----------
response | Количество бонусов (в гривнах/рублях), которое стало у клиента после внесения изменений
...
```php
$url = 'https://demo.joinposter.com/api/clients.changeClientPayedSum?' .
  'format=json&token=745d516e1b9320ed85b84d5bfda14148';
...
Запрос позволяет изменить общую сумму покупок клиента. В качестве количества необходимо прислать дельту, а не окончательное значение.
...
Параметр | Описание
--------- | -----------
client_id | id клиента, для которого меняем общую сумму покупок
count | Сумма покупок (в гривнах/рублях), которую надо начилить клиенту, если count положительный, или списать, если count отрицательный
block_webhook | Если вы слушаете вебхуки по клиентам, то будет полезно прислать этот параметр в виде true, чтобы не получить в ответ вебхук об измении данных этого клиента
...
Параметр | Описание
--------- | -----------
response | Общая сумма покупок (в гривнах/рублях), которая стала у клиента после внесения изменений